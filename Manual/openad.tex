...
\lstinputlisting[frame=single]{Code/OneMinute/head.f90}
\end{minipage}
\begin{minipage}[t]{.48\linewidth}
\lstinputlisting[frame=single]{Code/OneMinute/head.prepped.f90}
\end{minipage}
\caption{Simple example code (left), prepared for differentiation (right)}\label{fig:OneMinute}
\end{figure}
...
\lstinputlisting[frame=single]{Code/OneMinute/driver.f90}
\end{minipage}
...
to generate a code version for forward mode (controlled by the flag \lstinline{-m f}).
The script produces progress output like the one shown in \reffig{fig:oneMinuteStatusOutput}.
...
Lines 25,26 contain the derivative of $\tan(x)$ which is $\tan(x)^2+1$.
...
The call to \lstinline{sax} performs the actual propagation operation
$$
...
\caption{Progress messages from \lstinline{openad}.}\label{fig:oneMinuteStatusOutput}
%\end{wrapfigure}
\end{figure}
A simple driver that calls the transformed \lstinline{head(x,y)}
is shown in \reffig{fig:OneMinuteDriver}.
The output generated by the driver is shown in  \reffig{fig:oneMinuteOutput}.
%\begin{wrapfigure}{r}{.5\textwidth}
\begin{figure}
\fbox{
\begin{minipage}{.95\linewidth}
...
\caption{Output from \lstinline{driver} in \reffig{fig:OneMinuteDriver}.}\label{fig:oneMinuteOutput}
%\end{wrapfigure}
\end{figure}
\begin{figure}[h]
\fbox{
...
\scriptsize
\verbatiminput{Code/OneMinute/Makefile}
\end{minipage}
}
\caption{Contents of a \lstinline{Makefile} to compile and link the example}\label{fig:OneMinuteMake}.
\end{figure}
\begin{figure}
\lstinputlisting[frame=single,firstline=2]{Code/OneMinute/head.prepped.xb.x2w.w2f.pp.f}
...
\begin{figure}
\begin{minipage}{.98\linewidth}
\lstinputlisting[frame=single]{Code/OneMinuteReverse/driver.f90}
\end{minipage}
\caption{Driver routine for the transformed code  shown in \reffig{fig:OneMinuteReverseTransformed} }\label{fig:OneMinuteDriver}
%\end{wrapfigure}
\end{figure}
...
\caption{Progress messages from \lstinline{openad}.}\label{fig:oneMinuteReverseStatusOutput}
%\end{wrapfigure}
\end{figure}
A simple driver that calls the transformed \lstinline{head(x,y)}
...
\caption{Output from \lstinline{driver} in \reffig{fig:OneMinuteReverseDriver}.}\label{fig:oneMinuteOutput}
%\end{wrapfigure}
\end{figure}
\begin{figure}[h]
\fbox{
\begin{minipage}{.95\textwidth}
\scriptsize
\verbatiminput{Code/OneMinuteReverse/Makefile}
\end{minipage}
}
\caption{Contents of a \lstinline{Makefile} to compile and link the example}\label{fig:OneMinuteRverseMake}.
\end{figure}
\begin{figure}
\lstinputlisting[frame=single]{oneMinuteTaping.f}
\lstinputlisting[frame=single]{oneMinuteAdjoint.f}
\caption{Transformed code for taping (left) and the adjoint execution (right) for the input shown in \reffig{fig:OneMinute} (left)}\label{fig:OneMinuteReverseTransformed}
\end{figure}
...
\refsec{sec:dab} followed invoking\\[1ex]   
