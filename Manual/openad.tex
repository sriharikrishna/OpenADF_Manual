...
\author{J.~Utke  \\
	U.~Naumann \\
	{\small	\em Draft compile date \today }}
\date{ }
...
\end{itemize}
\item We assume the GNU c/c++/f77 compilers versions 3.3.x or newer. 
...
% -----------------------------------------------------------------------------------------
\section{OpenAD Environment}\label{sec:dab}
As needed during automated download of the components and the build process we 
need some environment variables to be set up to ease scripting and setting up make files.
From now on we assume the following steps have been taken to setup the environment.  
\begin{enumerate}
\item go into the \OpenADF\ source tree which was retrieved as described above:\\
\code {cd OpenAD}
\item set the environment for building and using \OpenADF\ for
\begin{itemize}
\item shell/ksh/bash etc users with\\
