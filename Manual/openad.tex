...
% -----------------------------------------------------------------------------------------
\section{Running the tool chain with the \lstinline{openad} script}
...
The most common  setups are encapsulated in a Perl script. 
...
\lstinline{OpenAD/tools/openad/openad}.
Invoking it with the \lstinline{-h} option displays the 
script usage of which the mode choices are the most important.
\begin{description}
\item[\lstinline{--bb-rev}] the default mode that produces adjoint code as explained in \refsec{sec:BBRev},
\item[\lstinline{--bb}] the tangent-linear mode as explained in \refsec{sec:BBPreacc}, and
\item[\lstinline{--memopstradeoff}] the tangent-linear mode with heuristics addressing data locality and operations count for the elimination sequence as explained in \refsec{sec:MMTradeOff}.
\end{description}
As an example we use a code assumed to be in a file called head.f and transform this code 
by invoking \lstinline{openad head.f}
which produces the adjoint version of \lstinline{head.f} emitting the following messages.\\
...
The following illustrates the explicit execution of the tool chain elements that could be implemented 
in a \lstinline{Makefile} where we use successive file name postfixes to 
...
\begin{minipage}{.95\textwidth}
\scriptsize
The binary is located at \\\lstinline{Open64/osprey1.0/targ_ia32_ia64_linux/crayf90/sgi/mfef90}.\\
The \lstinline{-F} flag enables C preprocessing if needed, the \lstinline{-ffixed} indicates fixed format input.
\end{minipage}
}\\\hline
...
\begin{minipage}{.95\textwidth}
\scriptsize
The binary is located at \\\lstinline{OpenADFortTk/OpenADFortTk-i686-Linux/bin/whirl2xaif}.\\
The \lstinline{-o} flag allows to specify an output file, the default is \lstinline{stdout}.
\end{minipage}
}\\\hline
...
\begin{minipage}{.95\textwidth}
\scriptsize
The binary \lstinline{<trans>} is one of\\
\lstinline{xaifBooster/algorithms/BasicBlockPreaccumulation/test/t}, see \refsec{sec:BBPreacc}\\
\lstinline{xaifBooster/algorithms/MemOpsTradeoffPreaccumulation/test/t}, see \refsec{sec:MMTradeOff}\\
\lstinline{xaifBooster/algorithms/BasicBlockPreaccumulationReverse/test/t}, see \refsec{sec:BBRev}\\
Each of the binaries prints a self explanatory usage message when invoked without any arguments. 
The intrinsics catalogue \lstinline{<iCat>} can be found in \\
\lstinline{xaif/schema/examples/inlinable_intrinsics.xaif} 
\end{minipage}
}\\\hline
...
\begin{minipage}{.95\textwidth}
\scriptsize
The binary is located at \\\lstinline{OpenADFortTk/OpenADFortTk-i686-Linux/bin/xaif2whirl}.\\
The optional \lstinline{--structured} flag indicates structured control flow, as required for 
...
\begin{minipage}{.95\textwidth}
\scriptsize
The binary is located at \\\lstinline{Open64/osprey1.0/targ_ia32_ia64_linux/whirl2f/whirl2f}.\\
It internally loads \lstinline{whirl2f_be} located in the same directory.
\end{minipage}
}\\\hline
...
\begin{minipage}{.95\textwidth}
\scriptsize
The script is located at \\\lstinline{OpenADFortTk/tools/multi-process/multi-pp.pl}.\\
For tangent linear models the postprocessor requires the command line flag \lstinline{-f} while 
for inlining the default file name \lstinline{ad_inline.f} can be changed with the \lstinline{-i} flag and for template expansion
the default template file name \lstinline{ad_template.f} can be changed with the \lstinline{-t} flag. 
\end{minipage}
}\\\hline
\end{longtable}
