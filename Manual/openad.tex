...
There is a slightly different \lstinline{Makefile} and \lstinline{driver} for 
the reverse mode.
...
\scriptsize\lstinputlisting[frame=single,lastline=17]{rtPropagationRoutines.txt}
\end{minipage}
\hspace{.02\linewidth}
\begin{minipage}[t]{.31\linewidth}
\scriptsize\lstinputlisting[frame=single,firstline=18,firstnumber=18,lastline=32]{rtPropagationRoutines.txt}
\end{minipage}
\hspace{.02\linewidth}
\begin{minipage}[t]{.31\linewidth}
\scriptsize\lstinputlisting[frame=single,firstline=33,firstnumber=33]{rtPropagationRoutines.txt}
\end{minipage}
...
\subsubsection{Reversal with Revolve}\label{sec:revolve}
...
To adapt a given loop to revolve one should use the templates given in the above examples. 
Care should be taken with the interpretation of the \lstinline{iteration} member 
of an \lstinline{rvAction} instance. Within the Revolve implementation the loop iterations 
are assumed to run from 0 to $s-1$  where $s$ is the argument value 
given to \lstinline{rvInit} as \lstinline{steps}. 
The value $i$ of the \lstinline{iteration} member is to be interpreted as follows with respect 
to a {\em current} iteration state $c$ that is initialized to 0. 
\begin{itemize}
\item for \lstinline{rvRestore}: the argument checkpoint for  iteration $i$ is being restored; set $c=i$   
\item for \lstinline{rvForward}: perform iterations $[c,i-1]$ with $c$ as the loop variable, that is, after 
this  loop the value of $c$ should be $i-1$.  
\end{itemize}
Using this recipe $c$ will attain all the iteration values in $[0,s-1]$ until the completion of the 
Revolve loop and $i$ can be interpreted 
as the respective ``next'' iteration not yet executed. For values of \lstinline{rvAction%actionFlag} 
other than the two values listed above no meaning is assigned to \lstinline{iteration}.  
...
\lstinputlisting[frame=single,firstline=119,firstnumber=119,lastline=130]{Examples/OneMinute/head.prepped.pre.xaif}
...
% -----------------------------------------------------------------------------------------
\subsection{Canonicalization with \lstinline{preProcess.py} (incompl.)}\label{sec:Canonicalization}
...
\begin{figure}
\begin{minipage}[t]{.48\linewidth}
\lstinputlisting[basicstyle=,frame=single,language=Fortran,firstline=6,firstnumber=6]{Examples/SRCanonical/func.f90}
\end{minipage}
\hspace{.02\linewidth}
\begin{minipage}[t]{.48\linewidth}
\lstinputlisting[basicstyle=,frame=single,language=Fortran,firstline=15,firstnumber=15]{Examples/SRCanonical/func.pre.f90}
\end{minipage}
...
\hspace{.02\linewidth}
\begin{minipage}[t]{.48\linewidth}
\lstinputlisting[basicstyle=,frame=single,language=Fortran,firstline=8,firstnumber=8,lastline=13]{Examples/SRCanonical/func.pre.f90}
\end{minipage}
...
\lstinputlisting[basicstyle=,frame=single,language=Fortran,firstline=17,firstnumber=17]{Examples/MaxCanonical/func.pre.f90}
\end{minipage}
\hspace{.02\linewidth}
\begin{minipage}[t]{.48\linewidth}
...
\begin{figure}
\begin{minipage}[t]{.48\linewidth}
\lstinputlisting[basicstyle=,frame=single,language=Fortran,firstline=6,firstnumber=6]{Examples/ArgExprCanonical/func.f90}
\end{minipage}
\hspace{.02\linewidth}
\begin{minipage}[t]{.48\linewidth}
\lstinputlisting[basicstyle=,frame=single,language=Fortran,firstline=9,firstnumber=9]{Examples/ArgExprCanonical/func.pre.f90}
\end{minipage}
...
% -----------------------------------------------------------------------------------------
\subsection{Postprocessing with \lstinline{postProcess.py} (incompl.)}\label{sec:PostProcessor}
...
\small
\lstinputlisting[basicstyle=,frame=single,language=Fortran,firstline=43,firstnumber=43,lastline=44]{Examples/OneMinuteReverse/head.prepped.pre.xb.x2w.w2f.f}
\end{minipage}\\
for which we have a definition in \lstinline{ad_inline.f} as \\[1ex]
\hspace*{.05\textwidth}\begin{minipage}{.6\textwidth}
\small
\lstinputlisting[basicstyle=,frame=single,language=Fortran,firstline=48,firstnumber=48,lastline=56]{Examples/OneMinuteReverse/ad_inline.f}
\end{minipage}\\
...
\hspace*{.05\textwidth}\begin{minipage}{.6\textwidth}
\small
\lstinputlisting[basicstyle=,frame=single,language=Fortran,firstline=102,firstnumber=102,lastline=103]{Examples/OneMinuteReverse/head.prepped.pre.xb.x2w.w2f.post.f90}
\end{minipage}\\
...
shown in \reffig{fig:joint}. Each pre-state change applies to the callees of the current 
...
\subsubsection{Error Conditions}\label{sec:postProcErrors}
In addition to what is mentioned in \refsec{sec:canonErrors} the post processing 
phase may abort with:   
\begin{description}
\item[{\tt PostProcessError} : ] an error specific to the post processing logic; 
if the message does not spell out a specific problem with the user input it may indicate a 
problem in the implementation and should be reported to the bug tracking system via the \OpenADF\ website.
\end{description}
% -----------------------------------------------------------------------------------------
...
\begin{itemize}
\item write a \lstinline{Makefile} that uses the \lstinline{openad} script,
\item identify independent and dependent variables, and
\item write driver logic to setup the derivative computation and obtain the derivative values.
\end{itemize}
...
\item compute a whole Jacobian either iteratively or in vector mode, and 
\item control the call graph reversal mode.
\end{itemize}

\subsection{Flow in a Driven Cavity}\label{sec:fdcExample}
This example code was taken from the Minpack test problem ``flow in a driven cavity'' (FDC) \cite{Averick1992TMT}.
The source code for the problem can be found under \lstinline{$OPENADROOT/Examples/FDC/} %$
...
finite differences we have setups with driver source files and \lstinline{Makefile}s recursively invoked by the above command in the following subdirectories
\begin{description}
...
\end{description}
The respective executables are named\\[1ex]
\hspace*{.3cm}\lstinline{driver}{\em subDir}\\[1ex]
where {\em subDir} is one of the four choices given above.
One can compare the produced output and observe matching values for the Jacobian entries. 
We suggest to compare also  the driver source files and the \lstinline{Makefile}s
...
the \lstinline{init} routine but it is \underline{\em not part of the transformation} because we want to differentiate only 
...
Consequently, in one sweep we can obtain in forward mode one projection $\bmJ\dot{\bmx}$ or in reverse mode one projection $\bar{\bmy}^T\bmJ$. 
...
\subsection{Box Model}\label{sec:boxModelExample}
This code was provided by Patrick Heimbach\footnote{\url{http://heimbach.wordpress.com/}}. 
It is used for the solution of a (generalized) eigenvalue
problem arising in physical oceanography,
where
substantial effort in understanding the
ocean circulation's role in the
variability of the climate system,
on time scales of decades to millennia and beyond,
is being directed at investigating
the so-called 'thermohaline' circulation (THC).
This refers to the contribution to
the ocean circulation which is driven by density gradients
and thus controlled by temperature and salinity properties
and its associated fluxes.
It plays a crucial role in connecting the surface
to the deep ocean through deep-water formation which occurs at some
isolated convection sites at high latitudes mainly in the subpolar
Atlantic ocean, such as the Labrador Sea and the
Greenland-Irminger-Norwegian (GIN) Seas.
...
The noteworthy differences to the example in \refsec{sec:fdcExample} are explained in the following.
...
% -----------------------------------------------------------------------------------------
\section{Shallow Water Model}\label{sec:shallow}
...
The application at hand is 
a shallow water model used in a
study \cite{Losch2003Bta} on bottom topography as a control variable in ocean
models. 
This code was originally written in Fortran77 but  there have
been a few Fortran90 extensions to the source. 
It contains some of
the basic language features also used in the much larger 
MIT general circulation model (MITgcm) \cite{mitgcmWeb,Adcroft2002TM}.
...
For a consistency check we looked at the relative discrepancy between 
finite differences and AD-derivatives. Even for a small setup with only 
160 gradient elements the discrepancies as plotted in \reffig{fig:shallowWaterFdAd} 
are significant. 
\begin{figure}
\centerline{\includegraphics[width=0.4\textwidth]{shallowWaterFdAd}}
\caption{Relative discrepancy between finite differences and AD derivatives for a 
small shallow water model setup with 160 gradient elements and a uniform perturbation factor}\label{fig:shallowWaterFdAd}
\end{figure}
...
to the tool which could be done by concatenating into a single file. 
...
a less efficient adjoint. {\bf Including ancillary sources may cause more conservative 
...
\refsecBS{sec:toyExample} explains the need for augmenting  
the ``driver'' logic for the model computation such that 
it carries out the seeding and extraction of the derivative values. 
...
of a large variety of reversal schemes. The model here involves a time stepping 
scheme controlled by a main loop. To facilitate explanations about the \OpenADF\ mechanisms in this example we will disregard
here the optimal Revolve scheme explained in \refsec{sec:revolve}.  
\OpenADF\ supports automatic detection of the 
data set to be checkpointed at a subroutine level. To use this feature the loop 
body is encapsulated into a inner loop subroutine denoted here for brevity \lstinline{I}, see \lstinline{loop_body_wrapper_inner.F}. 
To realize a nested checkpointing scheme
...
into an outer loop subroutine  \lstinline{O} (for brevity; see \lstinline{loop_body_wrapper_outer.F}) schematically shown in \reffig{fig:checkpointLoops} 
...
\end{figure}
The state changes can be encapsulated in four templates.
\begin{description}
\item{\lstinline{OADrts/ad_template.joint.f}}: joint mode template for \lstinline{top} and all its callees except \lstinline{O}
\item{\lstinline{OADrts/ad_template.joint_split_oif.f}}: for \lstinline{O}
\item{\lstinline{OADrts/ad_template.joint_split_iif.f}}: for \lstinline{I}                                               
\item{\lstinline{OADrts/ad_template.split.f}}: for all callees of \lstinline{I}
\end{description}  
The collection of downloadable test problems contains the 
model and the four subroutine templates.
\reffigBS{fig:checkpointLoops}(b) shows the \lstinline{cost} subroutine called from 
...
\lstinline{head.f90} are concatenated, the module name is changed to the \lstinline{LUstubs} module,
...
\begin{figure}[tb]
\begin{minipage}{\linewidth}
\small\lstinputlisting[basicstyle=,language=Fortran,frame=single]{Examples/LibWrappers/UmfPack_2.2_active/ADMsplit/Templates/solve.f90}
\end{minipage}
