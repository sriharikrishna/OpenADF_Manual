...
\small\lstinputlisting[frame=single,firstline=2]{Examples/OneMinute/head.prepped.xb.x2w.w2f.pp.f} %$
...
\begin{figure}
\lstset{language=[77]Fortran,basicstyle=}
\small\lstinputlisting[frame=single]{oneMinuteReverseTaping.f}
\small\lstinputlisting[frame=single]{oneMinuteReverseAdjoint.f}
...
\hspace*{.3cm}\lstinline{all/w2f__types.f90}\\[1ex] %$
...
\begin{figure}
\begin{minipage}[t]{.48\linewidth}
\scriptsize\lstinputlisting[frame=single,lastline=8]{rtPropagationRoutines.txt}
\end{minipage}
\hspace{.02\linewidth}
\begin{minipage}[t]{.48\linewidth}
\scriptsize\lstinputlisting[frame=single,firstline=9]{rtPropagationRoutines.txt}
\end{minipage}
...
\hspace*{.3cm}\lstinline{scalar/OAD_active.f90}
...
\hspace*{.3cm}\lstinline{simple/OAD_rev.f90}\\[1ex]
The split and joint reversal schemes using the state are enabled via the templates \\[1ex]
\hspace*{.3cm}\lstinline{simple/ad_template.joint.f}\\[1ex]
and\\[1ex]
\hspace*{.3cm}\lstinline{simple/ad_template.split.f}\\[1ex]
respectively.
...
\lstinputlisting[frame=single,firstline=102,lastline=123]{Examples/OneMinute/head.prepped.xaif}
...
\begin{Can} \label{can:funcToSub}
  All function calls  
  are canonicalized into subroutine calls, see \reffig{fig:funcToSubCall}.
\begin{figure}
\begin{minipage}[t]{.48\linewidth}
\lstinputlisting[basicstyle=,frame=single,language=Fortran]{Examples/SRCanonical/func.f90}
\end{minipage}
\hspace{.02\linewidth}
\begin{minipage}[t]{.48\linewidth}
\lstinputlisting[basicstyle=,frame=single,language=Fortran]{Examples/SRCanonical/func.pre.f90}
\end{minipage}
\caption{Canonicalizing a function (left, see  see file 
\lstinline{$OPENADROOT/Examples/SRCanonical/func.f90}), %$
to a subroutine (right,  see file 
...
dependent and independent declarartions, see \refsec{sec:open64FeBe}.
\begin{figure}
\begin{tabular}{cc}
\begin{minipage}{.4\textwidth}
\fontsize{8pt}{9pt}
