...
\begin{center}
  {\Large OpenAD/F: A modular open source tool for automatic differentiation \\ of Fortran codes}\\[1ex]
  {\tiny
...
generally be needed this option is usefull 
...
% -----------------------------------------------------------------------------------------
\subsection{Language Dependent Components (\OpenADFortTk)}\label{ssec:fortfe}
...
\item \whirlTof\ is the ``unparser'' that converts \whirl\ to
  Fortran, see \refsec{ssec:fortfe}
...
% -----------------------------------------------------------------------------------------
\subsubsection{Compiler Front-End Components (from \OpenSixtyFour)} \label{sssec:mfef}
...
developes \OpenSixtyFour\ \cite{open64Web} as a 
...
The mnemonic behind name is that
...
As indicated in \refsec{sec:ADIntro}, language independence for the transformation
engine is achieved by restricting the input language to the numerical core sublanguage.
The development of the transformation engine, however, can be streamlined by
(further) restricting the numerical core programs to a \emph{canonical}(i.e. simpler) form.
Please note that canonical form does \emph{not} restrict the expressiveness of the candidate input programs.
The canonical form merely serves to ensure that the transformation engine can be focused on
a smaller subset of semantically equivalent forms. The process of putting a program into canonical form
is called canonicalization. The rest of this subsection describes the canonical form and the mechanisms that
ensure that the canonical form is realized.

Part of the canonical form has already been described elsewhere, and is repeated here for convenience
...
\begin{Can} \label{can:assignSideEffectFree}
  ...
  For Fortran programs, single variable on the left-hand side is syntactically required.
  This side-effects free right hand side property obtains when the user-defined functions that have side effects
  are factored into a separate assignment statement. Rather than determine which user-defined functions have side
  effects, {\OpenAD} pragmatically factors \emph{all} user-defined functions out of the right hand sides of assignments.

  \begin{tabular}{p{24ex}cp{20ex}}
\verb|xxx = yyy * user(a,b)| & $\Rightarrow$ & 
   \begin{verbatim}
tmp = user(a,b)
xxx = yyy * tmp
   \end{verbatim}
...
\begin{Can} \label{can:assignElemental}
  The right-hand-side expression of an assignment consists only of elemental operations $\phi$ typically 
  defined in a programming language as built-in operators and intrinsics.

  The only other construct that can appear on the right-hand side of assignment statements would be user-defined
  functions. The transformation from Canonicalization~\ref{can:assignSideEffectFree} covers everything but
  the newly created temporary assignments of the form
  \begin{quote}
\begin{verbatim}
tmp = user(a,b)
\end{verbatim}
  \end{quote}
These assignment statements are converted to subroutine calls as per the requirement of Canonicalization~\ref{can:assignFunction}.
So, the combination of Canonicalization~\ref{can:assignSideEffectFree} and Canonicalization~\ref{can:assignFunction} ensures
that this canonical form requirement is met.
\end{Can}
\begin{Can} \label{can:assignFunction}
  ...
  The transformation to achieve this canonical form consists of changing the user-defined routine name to a
  canonically derived subroutine name, and adding the left-hand side to the argument list:

  \begin{tabular}{lcl}
   \verb|xxx = user(a,b)| & $\Rightarrow$ & \verb|call s_user(xxx,a,b)|
  \end{tabular}

\end{Can}

Additional canonicalizations found to be important are:

\begin{Can}\label{can:comBlock}
  Common blocks are converted to modules.

  The purpose of this canonicalization is to ensure proper initialization of global variables.

  The method of conversion is to simply declare the elements of the common block as actual variables
  of the associated module. Care must be taken to privatize and declare any symbolic size parameters
  for elements of the common block.

  \begin{tabular}{p{40ex}cp{30ex}}
\begin{verbatim}
   integer,paramter :: s=10
   real :: a,b

   common /foo/ a(s),b
\end{verbatim} & $\Rightarrow$ &
\begin{verbatim}
   module foo_cb
   private s
   integer,paramter :: s=10
...
\begin{Can}\label{can:param}
  ...
  can be augmented. Consequently, {\OpenAD} hoists all non-variable actual paramters into temporaries.

  \begin{tabular}{p{40ex}p{2ex}p{30ex}}
\begin{verbatim}
call foo(x*y)
\end{verbatim} & $\Rightarrow$ &
\begin{verbatim}
...
\begin{Can}\label{can:scalar}
  Derived type references are replaced.

  Derived type reference is not part of the numerical core, so any derived type reference
  must be transformed. The transformation (called \emph{scalarization}) consists of converting
  the derived type reference into a canonically named scalar variable. \bf{NOTE:} To ensure
  correctness, this transformation must be undone before the final augmented source is produced.

  \begin{tabular}{lcl}
\verb|x = s%v * y| & $\Rightarrow$ & \verb|x = s_v * y| \\
  \end{tabular}
...
{\color{red}
  ...
  [ how are canonicalizations implemented: some are done with separate canonicalizer, others done in whirl2xaif] \\
