...
\end{minipage}
\end{center}	
\caption{Pseudo code for $\bmJ_3\dot{\bmx}_3$ relating to the loop body in \reffig{fig:toy}}\label{fig:toyPreacc}
\end{figure}
...
it and produce transformed \xaif\ output as the result. All sources for \xaifBooster\ can be found under {\tt OpenAD/xaifBooster/}. The principal setup of the source tree is shown in \reftab{tab:dirStruct}.
\begin{figure}
...
Each algorithm has a driver {\tt t.pp} (compiled into a binary {\tt t}) 
found in {\tt algorithms/<the\_algorithm\_name>/test/} 
...
found in the sources in {\tt system/}.
Parsing is done through the Xerces C++ XML parser \cite{xercesWeb}
such that the XML element handler implementations, see {\tt system/src/XAIFBaseParserHandlers.cpp},
...
There is also a driver at this level found in {\tt system/test/t.cpp} used 
...
These representations are contained in the {\tt xaifBoosterLinearization::AssignmentAlg} instances associated with each assignment in the \xaif.
...
the code uses the interface coded in {\tt algorithms/CrossCountryInterface/}
