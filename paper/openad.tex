...
\item flexibility,
\item modularity, and
\item the use of open source compenents.
\end{itemize} 
...
supplied by \OpenAnalysis, see \refsec{sssec:openanalysis}.
...
read checkpoints at a subroutine level, see also \refsec{ssec:cgReversal}. 
This part of the transformation relies heavily on the results of side-effect analysis, see 
\refsec{sssec:openanalysis} and the inlinable subroutine call mechanism of 
...
\item \whirlToxaif\ is a bridge component that
  \begin{itemize}
  \item drives the various program analyses (see \refsec{sssec:openanalysis}),
...
In \refsec{sssec:xaif} we explain how the restriction to the numerical core 
...
\begin{Can}\label{can:param}
  ...
  can be augmented. 
  Consequently, {\OpenAD} hoists all non-variable actual paramters into temporaries.
  \begin{table}
    \begin{center}
      \begin{tabular}{cc}
        \begin{minipage}{.3\linewidth}
...
% -----------------------------------------------------------------------------------------
\subsubsection{Translating between \whirl\ and \xaif}\label{sssec:wtxxtw}
...
Derived type references are therfore {\em scalarized}. This consists of converting
...
explicit represenation in the  \xaif, 
...
\OpenAnalysis\ package, see \refsec{sssec:openanalysis}.
...
\item The demarkation of the various alternative subroutine bodies used in the subroutine template replacements.
\end{itemize}  

% -----------------------------------------------------------------------------------------
\subsubsection{Postprocessing}\label{sssec:PostProcessor}
The postprocessor performs the three tasks outlined at the end of \refsec{sssec:wtxxtw}.

\paragraph{Use of the Active Type}\label{sssec:activeType}
...
A rewrite in Python resuing the same Fortran parsing functionality of the canonicalizer is underway.
}
...
The type remains abstract because the accesses to the conceptual value and derivative components 
are represented as function calls \code{\_\_value\_\_(v)} and \code{\_\_deriv\_\_(v)} respectively.
The concretized versions created by the postprocessor for the current active type implementation are 
\code{type(active) v} for the declaration and simply \code{v\%v} for the value \code{v\%d} for 
the derivative component respectively. 

\paragraph{Inlinable Subroutine Calls}\label{sssec:inline}
...
\paragraph{Subroutine Templates}\label{sssec:templates}
...
call graph reversal. The reversal schemes introduced in \refsec{ssec:cgReversal}
...
\reffig{fig:codeTemplate}(b) shows a simple split-mode template.
The state is contained in \code{rev\_mode}, a static Fortran90 variable in module \code{OpenAD\_rev}
...
simplified joint mode template. 
...
retrieve and build all components. Then procede as follows.
\begin{enumerate}
...
Therefore user starts with steps 1 and 2 from \refsec{ssec:dab}. 
...
by adding the steps outlined in \refsec{ssec:manualPipeline} to a \code{Makefile}. 
...
1 & in (Fortran) & \code{head.f}\\\cline{2-3}
  & out (Fortran) & \code{head.canon.f} \\\cline{2-3}
  & command & \code{perl canon.pl head.f head.canon.f}\\\cline{2-3}
  & \multicolumn{2}{l|}{
\begin{minipage}{.8\textwidth}
\scriptsize
The script is located at \\{\tt OpenAD/OpenADFortTk/tools/multi-process/canon.pl}.
\end{minipage}
}\\\hline
2 & in (Fortran) & \code{head.canon.f} \\\cline{2-3}
  & out (\whirl) & \code{head.canon.B} \\\cline{2-3}
  & command & \code{mfef90 -F -ffixed head.canon.f}\\\cline{2-3}
  & \multicolumn{2}{l|}{
\begin{minipage}{.8\textwidth}
\scriptsize
The binary is located at \\{\tt OpenAD/Open64/osprey1.0/targ\_ia32\_ia64\_linux/crayf90/sgi/mfef90}.\\
...
invoked as {\tt perl multi-pp.pl [-i <inline definitions>] [-t <template definitions>] <fortran file>}.
